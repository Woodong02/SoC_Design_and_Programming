...
module rgb(
    input wire clk,              // [복구] 사용자님 원본 명세의 시스템 클럭 (TDMA 도메인)
    input wire opclk,            // LCD 픽셀 클럭 (12.5MHz)
    input wire rstn,
    input wire [9:0] HsyncCount, 
    input wire [9:0] VsyncCount, 
    input wire DE,

    // TDMA 혈관 (포트 이름 임의 변경 없음)
    input wire [2:0] slot,       
    input wire [21:0] clk_cnt,   
    input wire [15:0] DIV,       
    input wire GPIO_in,          
    ...
    always @(posedge clk or negedge rstn) begin
        if (!rstn) begin
            ...
            if (clk_cnt == 22'd0) begin
                bit_idx <= 6'd0;
                local_clk_cnt <= 16'd0;
            end else begin
                local_clk_cnt <= local_clk_cnt + 16'd1;
                if (local_clk_cnt >= (DIV - 16'd1)) begin
                    local_clk_cnt <= 16'd0;
                    if (bit_idx < 6'd50) bit_idx <= bit_idx + 6'd1;
                end
            end

            if (slot < 3'd8 && bit_idx < 6'd50) begin
    ...
    wire [7:0] local_x = x_pos - (grid_x * 160);    

    wire [5:0] read_bit_idx = (local_x * 3'd5) >> 4; 

    wire pixel_val = (current_box < 8 && read_bit_idx < 50) ? wave_buffer[current_box][read_bit_idx] : 1'b0;
    ...
    always @(posedge opclk) begin
        if (!rstn) begin
            ...
            if (current_box < 4'd8) begin
                if (is_cursor) begin
                    R <= 5'b11111; G <= 6'b000000; B <= 5'b00000; // 빨강 바늘
