...
module tb_slave22_master_harsh_link;
    ...
    task run_tracking_missed_frame_recovery;
        begin
            ...
            // S9: corrupt broadcast in TRACKING. The slave's prior TRACKING cycle
            // may already have a periodic holdover TX in flight (slot=0, the master
            // broadcast slot, normal slave behaviour). That leftover decode is NOT a
            // corrupt-frame-induced wrong-slot response. Mirror tb_slave22_master_link
            // L9 step2: clear any leftover decode AFTER the corrupt frame is fully
            // received, then require that no NEW recovery-slot (slot==NODE_ID) response
            // is produced by the corrupt frame itself.
            wait_to_next_master_frame_start();
            send_master_frame(8'h00, 8'h00, TB_GUARD_TICKS);
            master_valid_count = 0;
            expect_no_slave_tx("missed/corrupt tracking frame triggers no new tx", TB_NO_TX_OBSERVE);
            if ((master_valid_count != 0) && (last_valid_slot == TB_NODE_ID)) begin
                $display("[FAIL] missed/corrupt tracking frame produced new wrong-slot response valid_count=%0d last_slot=%0d last_clk_cnt=%0d",
                         master_valid_count, last_valid_slot, last_valid_clk_cnt);
                fail_count = fail_count + 1;
                must_pass_fail_count = must_pass_fail_count + 1;
            end else begin
                $display("[PASS] no new corrupt-frame-induced response completed during missed/corrupt tracking frame (leftover holdover slot=%0d ignored)",
                         last_valid_slot);
            end
    ...
    initial begin
        ...
        $display("[INFO] tb_slave22_master_harsh_link start");
        ...
        if (must_pass_fail_count == 0)
            $display("PASS: tb_slave22_master_harsh_link must-pass scenarios survived total_scenarios=%0d fail_count=%0d",
                     scenario_count, fail_count);
        else
            $display("FAIL: tb_slave22_master_harsh_link fail_count=%0d must_pass_fail_count=%0d total_scenarios=%0d",
