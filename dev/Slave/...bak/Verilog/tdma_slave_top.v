...
// 2-FF synchronizer on bus_a; soft_rst generates 1-cycle internal reset.
module tdma_slave_top #(
    ...
    // ── TDMA 물리 인터페이스 ──────────────────────────────────────────────
    input  wire        bus_a,       // Bus A 입력 (마스터→슬레이브 브로드캐스트)
    output wire        bus_b,       // Bus B 출력 (슬레이브→마스터), 비전송 시 0

    // ── 인터럽트 ──────────────────────────────────────────────────────────
    output wire        irq          // 액티브-하이, IRQ_MASK 적용 후 출력
);

    // ── 2-FF synchronizer for bus_a ────────────────────────────────────────
    reg tx_ff1, tx_ff2;
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) {tx_ff2, tx_ff1} <= 2'b0;
        else        {tx_ff2, tx_ff1} <= {tx_ff1, bus_a};
    end
    wire bus_a_sync = tx_ff2;
