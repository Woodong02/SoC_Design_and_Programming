...
├── 슬레이브 0: cycle_start + 0 × slot_ticks → TX (마스터 브로드캐스트와 동시, TX/RX 분리)
├── 슬레이브 1: cycle_start + 1 × slot_ticks → TX
│    ...
└── 슬레이브 N: cycle_start + N × slot_ticks → TX
     └── guard time 경과 → 다음 사이클 시작
```
...
| 상태 | 코드 | 조건 |
|------|------|------|
| INACTIVE | 000 | NODE_CNT 범위 밖 |
| NORMAL | 001 | FAULT_CNT < FAULT_TH |
| DATA_RECOVERY | 010 | DATA_FAULT 복구 중 (FAULT_CNT < FAULT_TH, 아직 0 미도달) |
| DATA_FAULT | 011 | FAULT_CNT ≥ FAULT_TH (슬롯 격리) |
...
| 파일 | 내용 |
|------|------|
| `01_phy_decisions(4).md` | 물리 계층: 케이블 분리, Manchester 인코딩, High-Z 아이들, 액티브 에지 기준 |
| `02_datalink_decisions(4).md` | 데이터링크: 50비트 프레임 구조, Hamming SEC-DED 7비트, 슬롯 길이 자동 계산 |
