...
set run_dir [file join $root sim slave_master_link xsim_run]
file mkdir $run_dir
cd $run_dir
...
set sources [list \
    ...
    [file join $root tb tb_slave_master_link.v] \
]
...
file delete -force tb_slave_master_link_sim.wdb
file delete -force webtalk.jou
file delete -force webtalk.log

exec [file join $vivado_bin "xvlog.bat"] -log xvlog.log {*}$sources
exec [file join $vivado_bin "xelab.bat"] tb_slave_master_link -debug typical -s tb_slave_master_link_sim -log xelab.log
exec [file join $vivado_bin "xsim.bat"] tb_slave_master_link_sim -runall -log xsim.log
