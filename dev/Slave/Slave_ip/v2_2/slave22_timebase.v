...
module slave22_timebase (
    ...
    localparam [2:0]  TB22_RESET       = 3'd0;
    localparam [2:0]  TB22_WAIT_FIRST  = 3'd1;
    localparam [2:0]  TB22_WAIT_SECOND = 3'd2;
    localparam [2:0]  TB22_HOLDOVER    = 3'd3;
    localparam [2:0]  TB22_TRACKING    = 3'd4;
    ...
    wire        good_interval_ok;
    wire [3:0]  miss_count_next_w;
    wire        miss_limit_exceeded;
    ...
    reg  [3:0]  miss_count_ff;
    reg  [3:0]  miss_count_next;
    reg         tx_trigger_ff;
    reg         tx_trigger_next;
    reg         rate_err_ff;
    reg         rate_err_next;
    ...
    assign rate_error_event = interval_ready &
                              ((interval_error > RATE_ERR_LIMIT_TICKS) |
                               (interval_error < -RATE_ERR_LIMIT_TICKS));
    assign good_interval_ok = interval_ready & ~rate_error_event;

    // miss_count_next as wire so FSM transitions can use post-increment value
    assign miss_count_next_w = good_interval_ok    ? 4'd0 :
                               rate_error_event     ? ((miss_count_ff == 4'hF) ? 4'hF :
                                                       (miss_count_ff + 4'd1)) :
                               miss_count_ff;
    assign miss_limit_exceeded = (miss_count_next_w > MISS_RESET_LIMIT);
    ...
    always @(*) begin
        ...
        case (state_ff)
            TB22_RESET: begin
                state_next = TB22_WAIT_FIRST;
            end

            TB22_WAIT_FIRST: begin
                if (good_broadcast_commit == 1'b1) begin
                    state_next = TB22_WAIT_SECOND;
                end else begin
                    state_next = TB22_WAIT_FIRST;
                end
            end

            TB22_WAIT_SECOND: begin
                if (rate_error_event == 1'b1) begin
                    if (miss_limit_exceeded == 1'b1) begin
                        state_next = TB22_WAIT_FIRST;
                    end else begin
                        state_next = TB22_HOLDOVER;
                    end
                end else if (good_broadcast_commit == 1'b1) begin
                    state_next = TB22_TRACKING;
                end else begin
                    state_next = TB22_WAIT_SECOND;
                end
            end
    ...
    always @(*) begin
        ...
        if (state_ff == TB22_RESET) begin
            bit_period_next = default_period;
        end else if (miss_limit_exceeded == 1'b1) begin
            bit_period_next = default_period;
    ...
    always @(*) begin
        ...
        if (state_ff == TB22_RESET) begin
            ...
            period_valid_next = 1'b0;
        end else if (good_interval_ok == 1'b1) begin
    ...
    always @(*) begin
        ...
        end else if (state_ff == TB22_RESET) begin
            slot_next = 3'd0;
        end else if (state_ff == TB22_WAIT_FIRST) begin
            slot_next = 3'd0;
        end
    end
    ...
    always @(*) begin
        miss_count_next = miss_count_next_w;
    end
