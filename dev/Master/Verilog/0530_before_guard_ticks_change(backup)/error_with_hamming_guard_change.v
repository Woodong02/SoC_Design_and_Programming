    ...
    if (!resetn) begin
      ...
      slot_timeout_cnt7 <= 8'b0;
        ...
        if(in_sig) begin
            ...
            3'd0: begin
                if((fixed_data[34:32] != slot) || (clk_cnt < 11'd50)) //when in_sig, the timing will exactly same with estimated last bit slave have given
                    slot_timeout_cnt0 <= 8'd255;
                else if( (clk_cnt < (11'd50 - (GUARD_TICKS>>2))) || (11'd50 + (GUARD_TICKS>>2) + (GUARD_TICKS>>1)) < clk_cnt)
