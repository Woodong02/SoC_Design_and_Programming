module master_top (
    ...
    wire [37:0] clk_cnt; //indicate currnet time in slot using cnt
    wire [2:0] slot; //current slot
    wire slot_change; //slot changed signal
    wire slot_pre_change; //slot changed signal (before 1clk)
    wire [31:0] DIV_p1 = DIV + 32'd1; //DIV cannot be 0. for saving
    wire tx_trigger = (slot_pre_change && slot==NODE_CNT)? 1'b1 : 1'b0; //cycle done signal for Master Tx
    wire [41:0] data_bus; //data bus for decode Hamming code
    wire sig_bus; //signal for data bus out/in
    wire preamble_err; //sinal for invalid preamble bit
    wire [1:0] rx_stat; //to detect Data Frame Slave Tx in correct time
    wire resetn = resetn_bt && ENABLE; //reset button with PS Enable will Manage negedge reset
    ...
    assign halt_cmd[7] = ((err_cnt7[31:24]+err_cnt7[23:16]+err_cnt7[15:8]) > FAULT_TH) ? 1'b1 : 1'b0; //Detect Err node
    ...
    wire [31:0] seg_slot_out = (DIP_SW[3:1]==3'd0) ? slot_out0 :
                           ...
                           (DIP_SW[3:1]==3'd6) ? slot_out6 : slot_out7; //7-segment will display each slot's data according to DIP SW
                           
    wire [31:0] seg_err_out; //For watch All nodes' status through 7-segment.
    ...
    wire [31:0] seg_in = DIP_SW[0] ? seg_slot_out : seg_err_out; //Dispay slot_out or node status
